...
CarControl::CarControl()
{
    carPos.x = 120;
    carPos.y = 300;
    steer_publisher = node_obj1.advertise<std_msgs::Float32>("Team908_steerAngle", 10);
    speed_publisher = node_obj2.advertise<std_msgs::Float32>("Team908_speed", 10);
...

    {
        ...
        while (right[i] == DetectLane::null)
        {
            ...
            if (i < 0){
                ...
                speed.data = velocity;
                 cout << "error_right: " << angle.data<<endl;
                  cout << " velocity1: " <<  speed.data<<endl;
                  steer_publisher.publish(angle);
                 speed_publisher.publish(speed);
            return;
            } 
